...
describe('formHandler', () => {
  ...
  describe('handleFormSubmit', () => {
    beforeEach(() => {
      document.body.innerHTML = `
        <form id="cosplayForm" onsubmit="handleFormSubmit(event)">
          ...
          <select id="category"><option value="cosplay">Cosplay</option></select>
    ...
    test('should hide the form after submission', () => {
      const event = { preventDefault: jest.fn() };
      handleFormSubmit(event);
    ...
    test('should remove "hidden" class from success message', () => {
      const event = { preventDefault: jest.fn() };
      handleFormSubmit(event);
    ...
    test('should scroll success message into view', () => {
      const event = { preventDefault: jest.fn() };
      const msg = document.getElementById('successMessage');
  ...
  describe('validateForm', () => {
    ...
    beforeEach(() => {
      document.body.innerHTML = `
        <form id="testForm">
          ...
          <select id="category"><option value="">-- Scegli --</option><option value="cosplay">Cosplay</option></select>
        </form>
      `;
    ...
    test('should return valid when all fields are filled correctly', () => {
      ...
      form.querySelector('#category').value = 'cosplay';
      const result = validateForm(form);
    ...
    test('should report category error when no option selected', () => {
      form.querySelector('#name').value = 'Mario';
      form.querySelector('#email').value = 'mario@example.com';
      // category stays at default empty value
      const result = validateForm(form);
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('category');
    });
